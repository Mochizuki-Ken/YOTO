...
def SaveImages(IMAGE,Folder_Name:str,Id:str,Format:str):
    ...
    IMAGE.save(f'{PROJECT_PATH}/ObjectImage/{Folder_Name}/{Id}.{Format.lower()}', 'PNG')
    IMAGE.close()
    return 0
...
def RemoveImageBackGround(Id:str,Folder_Name:str,File_Name:str):
    ...
    IMAGE = Image.open(f"{PROJECT_PATH}/Input/Images/{Folder_Name}/{File_Name}")
...
def RemoveAllImagesBackGround():
    FolderList:list = os.listdir(f"{PROJECT_PATH}/Input/Images/")
    print(FolderList)
    for Folder in FolderList:
        Folder_Path = os.listdir(f"{PROJECT_PATH}/Input/Images/{Folder}")
        path=os.path.join(PROJECT_PATH+"/ObjectImage/", Folder) 
        os.mkdir(path) 
        Id=1
